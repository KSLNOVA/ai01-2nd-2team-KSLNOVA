...
function App() {
  ...
  useEffect(() => {
    ...
    const defaultFeedback =
      FEEDBACK_PRESETS[exercise]?.[0] || `${exercise} 자세 분석 예시: 코어를 조금 더 조여 주세요.`;
  ...
  useEffect(() => {
    ...
    const assignVoice = () => {
      ...
      setVoices(voices);
      const pick =
        voices.find((v) => v.lang?.startsWith('ko') && /male|man|남성|boy/i.test(v.name || '')) ||
        voices.find((v) => v.lang?.startsWith('ko') && /Wavenet|Standard/i.test(v.name || '')) ||
        ...
        <section style={styles.heroCard}>
          ...
          <div style={styles.heroStatsGrid}>
            ...
            <div style={styles.statCard}>
              ...
              <div style={styles.statValue}>{EXERCISE_FOCUS[exercise]?.[0] || '폼 안정성'}</div>
              <div style={styles.statMeta}>{EXERCISE_FOCUS[exercise]?.[1] || '호흡 유지'}</div>
            </div>
          </div>
        </section>
